...
\subsubsection{결과 확인 방법}
\begin{itemize}
    \item \textbf{이메일 알림}: 등록된 학교 이메일로 승인/거부 결과 수신
    \item \textbf{GitHub Education 사이트}: \url{https://github.com/education}에서 신청 이력 및 결과 확인 가능
    \item \textbf{처리 기간}: 일반적으로 영업일 기준 7일 이내
\end{itemize}

\subsubsection{승인된 경우}
\begin{itemize}
    \item 승인 완료 이메일 수신
    \item \textbf{중요}: 승인 후 \textbf{72시간 이후}에 혜택이 활성화됨
    \item 활성화 후 GitHub Copilot 등 개발자 도구팩 사용 가능
\end{itemize}

\subsubsection{거부된 경우}
신청이 거부될 경우 당황하지 않고 다음 단계를 따르세요:

\begin{enumerate}
    \item \textbf{거부 사유 확인}
    \begin{itemize}
        \item 이메일 또는 GitHub Education 사이트에서 거부 사유 확인
        \item 일반적인 거부 사유:
        \begin{itemize}
            \item 학교 소속 증빙 자료 불충분
            \item 학생증 사진 불선명
            \item 학교 이메일 미인증
            \item 재학 기간 증빙 부족
        \end{itemize}
    ...
    \item \textbf{보완 후 재신청}
    \begin{itemize}
        \item 거부 사유에 해당하는 자료를 보완
        \item 더 선명한 학생증 사진 또는 재학증명서 준비
        \item 동일한 절차로 재신청 진행
    \end{itemize}
    
    \item \textbf{재신청 팁}
    \begin{itemize}
        \item 학생증과 함께 재학증명서를 추가 제출
        \item 사진 촬영 시 밝은 곳에서 글씨가 잘 보이도록 촬영
        \item 학교 포털 로그인 화면 캡처 등 추가 증빙 자료 활용
    \end{itemize}
\end{enumerate}

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!50!black, title=신청 이력 확인 방법]
\begin{enumerate}
    \item \url{https://github.com/education} 접속
    \item GitHub 계정으로 로그인
    \item 이전 신청 내역, 거부/승인 메시지 전체 확인 가능
\end{enumerate}
\end{tcolorbox}
