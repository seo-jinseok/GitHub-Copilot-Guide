...
\subsection{주요 특징}
\begin{itemize}
    \item \textbf{무료 라이선스 제공}: 대학 구성원(교수, 학생)에게는 월 약 15,000원 상당의 \textbf{GitHub Copilot Pro} 기능이 무료로 제공됩니다.
    \item \textbf{모델 통합 플랫폼}: GPT-5, Claude, Gemini 등 다양한 최신 AI 언어 모델을 하나의 플랫폼에서 선택하여 사용할 수 있습니다.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{@{}l X X@{}}
        ...
        \midrule
        \textbf{비용} & 무료 & \textbf{무료} (연 18만원 상당 지원) \\
        \midrule
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{인증 앱(Authenticator)}: 스마트폰에 \textit{Microsoft Authenticator} 또는 \textit{Google Authenticator} 앱을 사전에 설치하십시오.
    \begin{itemize}
        \item 앱스토어(iOS) 또는 Play스토어(Android)에서 ``Authenticator''로 검색하여 설치합니다.
        \item 별도의 회원가입 없이 설치만 하면 됩니다. GitHub 계정 연동은 아래 2단계 인증 설정 과정에서 QR 코드 스캔으로 진행합니다.
    \end{itemize}
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item 2단계 인증(2FA)이란 비밀번호 외에 스마트폰 앱으로 한 번 더 본인 확인을 거치는 보안 방식입니다.
        \item GitHub에 로그인 한 후, 우측 상단 프로필 $\rightarrow$ \textbf{Settings} $\rightarrow$ \textbf{Password and authentication}
        \item \textbf{Two-factor authentication}을 활성화하면 QR 코드가 화면에 표시됩니다.
        \item 사전에 설치한 Authenticator 앱을 실행하고, 앱 내 \textbf{+ 버튼}을 눌러 해당 QR 코드를 스캔합니다. 스캔 후 앱에 표시되는 6자리 숫자를 GitHub 화면에 입력하면 연동이 완료됩니다.
        \item \textit{주의: 생성되는 복구 코드는 반드시 별도로 저장하십시오. 메모장에 저장 후 USB나 클라우드에 백업하거나, 인쇄하여 안전한 곳에 보관하시기 바랍니다.}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item 브라우저의 위치 공유 권한을 허용하여 현재 위치를 인증합니다. 이는 실제로 해당 대학 소속인지 확인하기 위한 절차이며, 거부 시 신청이 거절될 수 있습니다.
    \end{itemize}
    \item \textbf{서류 제출}: 준비한 영문 증명서를 업로드하거나 카메라로 촬영하여 제출합니다. \textbf{Submit} 버튼을 누르면 신청이 완료됩니다.
\end{enumerate}

\section{설정 및 활용 방법}
승인이 완료되면 등록된 이메일로 알림이 발송되며, 웹 브라우저(\href{https://github.com/copilot}{copilot.github.com})에서 즉시 AI 기능을 사용할 수 있습니다.
...
\subsection{처음 대화 시작하기}
Copilot 설정이 완료되면 다음과 같이 AI와 대화를 시작할 수 있습니다.
\begin{enumerate}
    \item \href{https://github.com/copilot}{copilot.github.com}에 접속하면 화면 중앙에 \textbf{대화 입력창}이 표시됩니다.
    \item 입력창에 질문이나 요청 사항을 한국어로 자유롭게 입력하고 \textbf{Enter} 키를 누르면 AI가 응답합니다.
    \item \textbf{파일 첨부}: 입력창 좌측의 \textbf{+ 아이콘}을 클릭하여 문서 파일을 첨부할 수 있습니다.
    \item \textbf{이미지 분석}: 이미지 파일(JPG, PNG 등)을 첨부하면 AI가 사진, 차트, 도표 등을 분석해 줍니다.
    \item \textbf{모델 변경}: 입력창 하단의 모델명을 클릭하면 다른 AI 모델로 전환할 수 있습니다.
\end{enumerate}

\vspace{0.5em}
\noindent
\textit{※ 첫 질문 예시: ``이 문서를 요약해 주세요'', ``다음 내용을 공문 형식으로 작성해 주세요'', ``이 표의 데이터를 분석해 주세요'' 등}
...
\addlinespace
\textbf{프리미엄} & Claude Sonnet 4.5 & \textbf{한국어 문서 작성 최적}. 가장 자연스러운 한국어 문장을 생성하는 모델입니다. 보고서, 기획서, 공문서 작성에 탁월하며, 복잡한 문제 해결 능력도 뛰어납니다. \\
\addlinespace
