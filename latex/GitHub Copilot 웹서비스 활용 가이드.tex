...
\newcommand{\docversion}{최종~수정:~\int_use:N \g_kst_year_int.\int_use:N \g_kst_month_int.\int_use:N \g_kst_day_int
...
\subsection{주요 특징}
\begin{itemize}
    \item \textbf{무료 라이선스 제공}: 대학 구성원(교수, 학생)에게는 월 약 15,000원(연 18만원) 상당의 \textbf{GitHub Copilot Pro} 기능이 무료로 제공됩니다.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{@{}l X X@{}}
        ...
        \addlinespace
        \textbf{대화 한도} & 기본 모델 월 50회 제한 & \textbf{기본 모델 무제한} \newline 프리미엄 모델 월 300회 (매월 1일 초기화, 일반 업무에 충분한 양) \\
        \addlinespace
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{주의: 생성되는 복구 코드(Recovery codes)는 반드시 별도로 저장하십시오. 메모장에 저장 후 USB나 클라우드에 백업하거나, 인쇄하여 안전한 곳에 보관하시기 바랍니다.}
    \end{itemize}
