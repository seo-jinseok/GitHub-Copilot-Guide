...
| AI 이름 | 만든 회사 | 특징 | 사용 제한 |
|---------|----------|------|--------------|
| **GPT-4.1** | OpenAI | 고성능 추론 모델 | **월 50회** (통합) |
| **GPT-5 mini** | OpenAI | 경량화된 최신 모델 | **월 50회** (통합) |
| **Claude 3.5 Sonnet** | Anthropic | (일부 사용자에게 제공될 수 있음) | **월 50회** (통합) |

> **💡 참고:** 무료 사용자는 모든 모델 사용량을 합쳐서 **매월 50회**까지만 대화할 수 있습니다.

---

### 🎓 대학 구성원(학생/교수)이 쓸 수 있는 AI

**GitHub Education 혜택**을 받으면 훨씬 더 많은 최신 AI를 사용할 수 있습니다! 모델은 크게 **기본(Standard)**, **프리미엄(Premium)**, **효율(Efficient)** 모델로 나뉩니다.

#### 📱 기본 모델 (Standard) - **무제한 사용!** 🎉

가장 많이 쓰게 될 모델들입니다. 횟수 제한 없이 마음껏 쓰세요.

| AI 이름 | 만든 회사 | 특별한 점 | 사용 제한 |
|---------|----------|----------|--------------|
| **GPT-4o** | OpenAI | 멀티모달(이미지 인식 등) 능력, 빠른 성능 | **무제한 (0x)** |
| **GPT-4.1** | OpenAI | 고성능 추론 모델 | **무제한 (0x)** |
| **GPT-5 mini** | OpenAI | 경량화된 GPT-5, 빠르고 똑똑함 | **무제한 (0x)** |
| **Grok Code Fast 1** | xAI | 매우 빠른 코드 생성 | **무제한 (0x)** |
| **Raptor mini (Preview)** | - | 경량 추론 모델 | **무제한 (0x)** |

#### 🚀 프리미엄 모델 (Premium) - **월 300 크레딧**

최고 성능의 모델들입니다. 매월 제공되는 **300 크레딧** 내에서 사용할 수 있습니다.

| AI 이름 | 만든 회사 | 특별한 점 | 비용 (차감) |
|---------|----------|----------|--------------|
| **Claude Haiku 4.5** | Anthropic | 빠르고 비용 효율적 | **0.33 크레딧** |
| **Claude Sonnet 4 / 4.5** | Anthropic | 자연스러운 문장력, 코딩 능력 탁월 | **1 크레딧** |
| **Gemini 2.5 Pro / 3 Pro** | Google | 방대한 정보 처리(긴 문서)에 강력함 | **1 크레딧** |
| **GPT-5** | OpenAI | OpenAI의 최신 플래그십 모델 | **1 크레딧** |
| **GPT-5-Codex** | OpenAI | 코딩 작업에 특화된 모델 | **1 크레딧** |
| **GPT-5.1 (Preview)** | OpenAI | 최신 실험적 기능 | **1 크레딧** |
| **GPT-5.1-Codex-Mini** | OpenAI | 가벼운 코딩 작업용 | **0.33 크레딧** |

> **💡 팁:** **0.33 크레딧** 모델(Claude Haiku 4.5 등)을 사용하면 300 크레딧으로 **약 900회**까지 대화할 수 있습니다!

> **🎁 핵심 혜택:**
> 1. **GPT-4o가 무제한**입니다! 이것만으로도 대부분의 작업이 가능합니다.
> 2. **월 300 크레딧**은 GPT-5나 Claude 3.5 Sonnet 같은 최고급 모델을 하루 10번씩 매일 쓸 수 있는 양입니다.
> 3. 크레딧을 다 써도 **기본 모델(GPT-4o 등)은 계속 무제한**입니다. 사실상 **완전 무제한**이나 다름없습니다.
...
graph TD
    ...
    B -->|일반적인 대화, 이미지 분석| C[GPT-4o (무제한) 추천]
    B -->|빠른 코드 생성| D[Grok Code Fast 1 (무제한) 추천]
    B -->|보고서/글쓰기/코딩| E[Claude Sonnet 4.5 (1x) 추천]
    B -->|긴 문서 분석| F[Gemini 2.5 Pro (1x) 추천]
    B -->|최신 범용 작업| G[GPT-5 (1x) 추천]
...
| 하고 싶은 일 | 추천 AI | 이유 | 비용 |
|------------|---------|------|------|
| **일상적인 질문/업무** | **GPT-4o** | 빠르고 똑똑하며 무제한이니까 | **무료 (0x)** |
| **이메일 초안 작성** | **Claude Sonnet 4.5** | 문장이 가장 자연스러움 | 1 크레딧 |
| **논문/보고서 요약** | **Gemini 2.5 Pro** | 긴 글을 잘 읽음 | 1 크레딧 |
| **복잡한 코딩** | **GPT-5-Codex** | 코딩 전문 모델 | 1 크레딧 |

> **💡 팁:** 평소에는 **GPT-4o(무제한)**를 쓰다가, 정말 중요한 문서나 어려운 질문이 있을 때만 **Claude Sonnet 4.5**나 **GPT-5**를 쓰세요. 그러면 크레딧 걱정 없이 최고 성능을 누릴 수 있습니다!
...
|------|--------------------------|---------------------------|
| **사용 가능한 AI** | GPT-4.1, GPT-5 mini 등 | **GPT-4o(무제한)** +<br/>GPT-5, Claude 3.5 등(월 300회) |
| **사용 제한** | **월 50회** (통합) | **무제한** (기본 모델)<br/>**월 300 크레딧** (프리미엄 모델) |
| **VS Code에서 사용** | 제한적 (월 2000줄 자동완성) | **무제한 자동 완성** |
| **데이터 보호** | 학습에 사용될 수 있음 | **학습에 절대 사용 안 함** 보장 |

**쉽게 말하면:**
- 무료 사용자: 한 달에 50번만 대화 가능 (아껴 써야 함)
- 대학 구성원: **GPT-4o를 무제한**으로 쓰면서, 필요할 때 **최고급 AI도 월 300번**까지 사용 가능! 🎉
