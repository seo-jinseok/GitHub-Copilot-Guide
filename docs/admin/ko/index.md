...
[🏠 홈](../../index.md) | [🌐 웹 가이드](../../common/ko/copilot_web.md) | [💻 VS Code 가이드](../../index.md#2부-vs-code로-전문가처럼-쓰기-설치-)
...
### **[Level 1: 입문 - AI 행정가의 첫걸음](#level-1-입문-ai-행정가의-첫걸음)**
  - [1. 준비하기: 나만의 디지털 집무실 만들기](#1-준비하기-나만의-디지털-집무실-만들기)
    ...
    - [1.5 생성형 AI 활용의 3대 원칙](#15-생성형-ai-활용의-3대-원칙)
...
**학습 방법**:
1. 각 섹션의 체크박스 `- [ ]`는 학습 진도를 표시하는 **시각적 가이드**입니다. (웹에서는 클릭할 수 없지만, 문서를 다운로드하시면 체크 가능합니다)
2. Workbook 실습은 반드시 직접 따라 하며 체득하세요.
3. Key Takeaways와 퀴즈로 핵심 내용을 복습하세요.

> **💡 Tip:** 이 문서를 로컬에 다운로드하여 마크다운 에디터(VS Code 등)에서 열면 체크박스를 직접 클릭하며 학습 진도를 관리할 수 있습니다!
...
## [Level 0: 준비] AI 불안 해소 클리닉 - 시작하기 전 오해 풀기

본격적인 학습에 앞서, 행정직원분들이 가장 많이 걱정하시는 3가지 오해를 먼저 풀고 가겠습니다.

### 1. 🔒 "내 정보가 유출되나요?" (보안)
> **"대학 내부 문서를 AI에 올려도 되나요?"**

많은 분들이 뉴스에서 본 '개인정보 유출 사례' 때문에 걱정하십니다. 하지만 우리가 사용하는 **기업용(Enterprise) 버전**은 다릅니다.
*   **무료 버전(일반용):** 사용자의 데이터를 학습에 사용하여 똑똑해집니다. (입력 금지 ❌)
*   **기업용 버전(Copilot):** **사용자의 데이터를 절대 학습하지 않습니다.** 입력한 데이터는 암호화되어 처리되고 즉시 폐기됩니다. (입력 가능 ✅)
*   **결론:** 학교에서 제공하는 계정은 안전합니다. 단, 주민등록번호 등 민감한 개인정보는 마스킹(***) 처리하는 습관을 들이는 것이 좋습니다.

### 2. 🤥 "AI는 거짓말쟁이?" (정확성)
> **"AI가 쓴 글을 그대로 믿어도 되나요?"**

아니요, 믿으시면 안 됩니다. AI를 **'일 잘하지만 가끔 엉뚱한 소리를 하는 똑똑한 인턴'**이라고 생각하세요.
*   **AI의 역할:** 초안 작성, 아이디어 제안, 단순 반복 작업 (작업의 80%)
*   **여러분의 역할:** 팩트 체크, 최종 검수, 책임 지기 (나머지 20%)
*   **결론:** AI는 '정답 자판기'가 아니라 '협업 파트너'입니다. 최종 결재권자는 여전히 여러분입니다.

### 3. 🤖 "내가 대체될까?" (일자리)
> **"AI가 내 일을 다 하면 저는 필요 없어지나요?"**

계산기가 나왔다고 수학자가 사라지지 않았고, 엑셀이 나왔다고 회계사가 사라지지 않았습니다.
*   **사라지는 것:** 복사-붙여넣기, 단순 요약, 오탈자 찾기 같은 '지루한 반복 업무'
*   **남는 것:** 기획, 소통, 판단, 의사결정 같은 '가치 있는 업무'
*   **결론:** AI가 여러분을 대체하는 것이 아니라, **"AI를 잘 쓰는 행정가가 AI를 안 쓰는 행정가를 대체"**하게 될 것입니다. 이 가이드를 통해 그 'AI 잘 쓰는 행정가'가 되어보세요.
...
### 1.5 생성형 AI 활용의 3대 원칙
- [ ] 학습 완료

도구 사용법을 익히기 전에, AI와 일하는 **'마음가짐'**을 먼저 정립해야 합니다. 이 3가지 원칙만 지켜도 AI 도입의 실패 확률을 90% 줄일 수 있습니다.

#### 원칙 1: 업무의 분업 (Delegation) - "누가 무엇을 할 것인가?"
AI는 만능이 아닙니다. **'똑똑하지만 사회생활 경험이 없는 인턴'**이라고 생각하세요.

| 구분 | 🤖 AI에게 맡길 일 (80%) | 👤 사람이 직접 해야 할 일 (20%) |
| :--- | :--- | :--- |
| **성격** | 초안 작성, 요약, 번역, 데이터 포맷 변환 | 최종 의사결정, 책임지는 일, 민감한 소통 |
| **예시** | "지난달 회의록 3줄 요약해줘" | "이 안건을 총장님께 보고할지 결정" |
| **예시** | "이 표를 엑셀 형식으로 바꿔줘" | "예산 삭감에 대한 정중한 거절 메일 작성" |

> [!TIP]
> **판단 기준:** "이 결과물에 대해 내가 책임질 수 있는가?" 책임질 수 없는 부분은 AI에게 맡기더라도 반드시 사람이 다시 봐야 합니다.

#### 원칙 2: 검증의 의무 (Verification) - "돌다리도 두들겨 보고 건너라"
초보자가 가장 많이 하는 실수는 AI가 내놓은 답을 **'그대로 복사해서 쓰는 것'**입니다. 이것은 사고로 이어지는 지름길입니다.

**Human-in-the-Loop (사람이 중심에 있는) 워크플로우:**
```mermaid
graph LR
    A[업무 지시] --> B(🤖 AI 생성);
    B --> C{👤 인간 검증};
...
*   **숫자 확인:** 예산, 날짜, 전화번호는 반드시 원본과 대조하세요.
*   **URL 확인:** AI가 만들어준 링크는 실제로 눌러봐야 합니다. (가짜 링크일 수 있음)
*   **팩트 체크:** "우리 학교 규정에 따르면..."이라고 AI가 말할 때, 실제 규정집을 찾아보세요.

#### 원칙 3: 일관성의 힘 (Consistency) - "한 목소리로 말하기"
하나의 큰 프로젝트(예: 대학혁신지원사업 계획서)를 여러 명이 쓰거나, 혼자서 며칠에 걸쳐 쓰다 보면 용어나 문체가 달라질 수 있습니다.

*   **문제 상황:** 
    *   페이지 1: "재학생" / 페이지 10: "수강생" / 페이지 50: "학습자"
    *   페이지 1: "~합니다." (해요체) / 페이지 20: "~함." (개조식)
*   **해결책:**
    *   **용어 정의:** 작업 시작 전에 AI에게 "이 문서에서는 학생을 '재학생'으로 통일해줘"라고 지시하세요.
    *   **문체 통일:** "이전 문서의 말투와 똑같이 작성해줘"라고 요청하세요.
    *   **VS Code 활용:** 여러 파일을 하나의 폴더(프로젝트)에 넣고 작업하면, Copilot이 전체 맥락을 이해하여 일관성을 유지해줍니다.
